...
|--------------------|-----|--------------|
| Generated `aer-route-v1` image and metadata from `silicon-bridge` | Select the external input address consumed by LIF/STDP | **Consumer wired**: 16 x 16-bit table, 4-bit addresses, MAX_HOPS=4, INIT image plus target-3 writes. The committed identity image is reproducible; cross-repo NIR producer work is tracked separately. |
