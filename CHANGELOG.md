...
- [`docs/resource-budget-n16.md`](docs/resource-budget-n16.md) refreshed against a real routed
  ...
  build emits `utilization_hier.rpt`, which attributes the total per module: `SocProtocolFsm`
  is ~61% of LUTs and ~68% of flip-flops (it holds both an active and a pending 16-word
  response snapshot), while `SocStatusLeds` costs only 49 LUTs / 55 FFs.
